...
def add_split_points(roberta, encoders_per_rank):
    for i in range(0, roberta.config.num_hidden_layers // encoders_per_rank):
        annotate_split_points(roberta, {f'roberta.encoder.layer.{i}': PipeSplitWrapper.SplitPoint.BEGINNING})
    annotate_split_points(roberta, {'lm_head': PipeSplitWrapper.SplitPoint.BEGINNING})
...
def wrap(model, training_args, pp_ranks):
    ...
    encoders_per_rank = (model.config.num_hidden_layers + num_of_ranks_for_encoders - 1) // num_of_ranks_for_encoders  # a divider of roberta.config.num_hidden_layers: [1, 2, 3, 4, 6, 12]
    # print(f"encoders_per_rank = {encoders_per_rank}")
    number_of_workers = emb_head + model.config.num_hidden_layers // encoders_per_rank  # 3 + a divider of roberta.config.num_hidden_layers: [4, 5, 6, 7, 9, 15]
