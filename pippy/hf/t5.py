...
def add_split_points(t5, xxcoders_per_rank):
    ...
    assert (t5.config.num_layers + t5.config.num_decoder_layers) % xxcoders_per_rank == 0
    encoders_per_rank = xxcoders_per_rank
    for i in range(0, (t5.config.num_layers + encoders_per_rank - 1) // encoders_per_rank):
    ...
    annotate_split_points(t5, {'lm_head': PipeSplitWrapper.SplitPoint.BEGINNING})
