...
def split_by_tags(gm: pippy.fx.GraphModule, tags: List[str]) -> pippy.fx.GraphModule:
    ...
    def flatten(x: pippy.fx.node.Argument) -> NodeList:
        """
    ...
    node_remapping: Dict[pippy.fx.Node, pippy.fx.Node] = {}

    # Mapping from node in original module or created submodules to
    # corresponding component.
    node_to_component: Dict[pippy.fx.Node, Component] = {}
    ...
    main_remapping: Dict[pippy.fx.Node, pippy.fx.Node] = {}

    # Output node of original module.
    output_node: Optional[pippy.fx.Node] = None

    # Create a component for each tag, we don't expect to create other components afterwards.
    for tag in tags:
        comp = Component(pippy.fx.Graph(), len(all_components), f"{tag}")
    ...
    for comp in all_components:
        ...
        if len(outs) == 1:
            ...
            for i, o in enumerate(comp.orig_outputs):
                ...
                main_remapping[o] = pippy.fx.Proxy(main_node)[i].node  # type: ignore[index]
    ...
    return pippy.fx.GraphModule(main_root, main_g)
