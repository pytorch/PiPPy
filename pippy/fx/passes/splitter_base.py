...
class _SplitterBase:
    ...
    def find_deps(self) -> Dict[pippy.fx.Node, NodeSet]:
        ...
        """
        deps: Dict[pippy.fx.Node, NodeSet] = defaultdict(set)
        for node in self.module.graph.nodes:
            if node.op not in CALLABLE_NODE_OPS:
                continue
    ...
    def _find_culprit(self, mod: pippy.fx.GraphModule, inputs: Tensors) -> str:
        """
    ...
    def _draw_graph_based_on_node_support(
        self, mod: pippy.fx.GraphModule, supported_nodes: NodeList
    ):
        color_map = {
            "default": "AliceBlue",
    ...
    def node_support_preview(self, dump_graph: bool = False):
        ...
        for node in self.module.graph.nodes:
            ...
            arg_dtypes = [
                get_dtype(arg) if isinstance(arg, pippy.fx.Node) else None
    ...
    def split_preview(self, dump_graph: bool = False):
        ...
        for node in split_mod.graph.nodes:
            if node.op == "call_module" and "acc" in node.target:
                ...
                def get_bytes(node: pippy.fx.Node):
    ...
    def find_reverse_deps(
        ...
        """
        result: Dict[pippy.fx.Node, NodeSet] = defaultdict(set)
    ...
    def update_reverse_deps_for_fusions(
        self, deps: Dict[pippy.fx.Node, NodeSet]
    ):
        processed_node = set()
    ...
    def put_nodes_into_subgraphs(self) -> List[Subgraph]:
        ...
        while current_cpu_nodes or current_acc_nodes:
            ...
            current_nodes = (
                current_acc_nodes if acc_subgraph else current_cpu_nodes
            )
            node = next(
    ...
    def tag(self, subgraphs: List[Subgraph]):
        ...
        for subgraph in subgraphs:
            ...
            for node in subgraph.nodes:
                if hasattr(node, "tag"):
                    raise FxNetSplitterInternalError(
                        f"Node {node} was already tagged"
                    )
                node.tag = tag  # type: ignore[attr-defined]
    ...
    def __call__(self) -> pippy.fx.GraphModule:
        subgraphs = self.put_nodes_into_subgraphs()
        subgraphs = self.remove_small_acc_subgraphs(subgraphs)
    ...
    def generate_split_results(self) -> SplitResult:
        ...
        return SplitResult(
            split_module, submodule_inputs, self.non_acc_submodule_name
        )
