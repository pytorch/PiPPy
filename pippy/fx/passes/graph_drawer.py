...
from pippy.fx.node import _get_qualified_name, _format_arg
...
if HAS_PYDOT:
    ...
    class FxGraphDrawer:
        """
        Visualize a pippy.fx.Graph with graphviz
        Basic usage:
        ...
        def __init__(
            ...
            for node in graph_module.graph.nodes:
                ...
                if not isinstance(leaf_node, pippy.fx.GraphModule):
                    continue
        ...
        def _get_node_style(self, node: pippy.fx.Node) -> Dict[str, str]:
            template = {
                "shape": "record",
                "fillcolor": "#CAFFE3",
                "style": '"filled,rounded"',
        ...
        def _get_leaf_node(
            self, module: torch.nn.Module, node: pippy.fx.Node
        ) -> torch.nn.Module:
            py_obj = module
