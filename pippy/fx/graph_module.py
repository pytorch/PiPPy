...
def reduce_graph_module(
    body: Dict[Any, Any], import_block: str
) -> torch.nn.Module:
...
def _copy_attr(
    ...
    *prefix, field = target.split(".")
    ...
    if isinstance(orig, torch.Tensor) and not isinstance(
        orig, torch.nn.Parameter
    ):
...
class _WrappedCall:
    ...
    def _generate_error_message(frame_summary: traceback.FrameSummary) -> str:
        ...
        err_and_after_err = "\n".join(
            all_src_lines[err_lineno : err_lineno + 2]
        )
...
class GraphModule(torch.nn.Module):
    ...
    def __init__(
        ...
        if isinstance(root, torch.nn.Module):
            ...
            raise RuntimeError(
                "Unsupported type " + str(root) + " passed for root!"
            )
    ...
    def graph(self, g: Graph) -> None:
        ...
        assert isinstance(
            g, Graph
        ), f"Expected a Graph instance, but got {type(g)}"
        self._graph = g
    ...
    def to_folder(
        ...
        tab = " " * 4
        custom_builtins = "\n".join(
            [v.import_str for v in _custom_builtins.values()]
        )
        model_str = f"""
import torch
{custom_builtins}
...
"""
        ...
        for module_name, module in self.named_children():
            ...
            if module_str is None:
                ...
                module_repr = (
                    module.__repr__().replace("\r", " ").replace("\n", " ")
                )
    ...
    def delete_all_unused_submodules(self) -> None:
        ...
        to_delete = [
            name for name, _ in self.named_modules() if name not in used
        ]
    ...
    def __reduce_package__(self, exporter: PackageExporter):
        ...
        python_code = self.recompile()
        import_block = _format_import_block(
            python_code.globals, exporter.importer
        )
    ...
    def nested_str(self) -> str:
        ...
        module_code = (
            f"class {self._get_name()}(torch.nn.Module):\n" + module_code
        )
