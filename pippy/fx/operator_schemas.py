...
def get_signature_for_torch_op(op: Callable, return_schemas: bool = False):
    ...
    signatures = [
        _torchscript_schema_to_signature(schema) for schema in schemas
    ]
    ...
    if getattr(signature_type, "__origin__", None) in {list, List}:
        ...
        def is_homogeneous_tuple(t):
            ...
            if t.__args__ == (
                ...
                return True
            return all(
                (c is Ellipsis) or issubclass(c, sig_el_type) for c in contained
            )
                    ...
                    for candidate_signature in torch_op_schemas:
                        ...
                        try:
                            ...
                            for (
                                arg_name,
                                arg_type,
                            ) in bound_types.arguments.items():
...
def _args_kwargs_to_normalized_args_kwargs(
    ...
    if any(
        p.kind not in supported_parameter_types for p in sig.parameters.values()
    ):
        return None
