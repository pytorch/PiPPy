...
from .node import Argument, base_types, map_aggregate  # pylint: disable=unused-import
...
class ProxyableClassMeta(type):
    ...
    symbolically traceable. For example::
        ...
        traced = pippy.fx.symbolic_trace(use_tensor_pair_ctor)
        print(traced.code)
        '''
...
class Tracer(TracerBase):
    ...
    of ``pippy.fx.symbolic_trace``. A call to ``symbolic_trace(m)`` is equivalent
    to ``Tracer().trace(m)``.
...
def wrap(fn_or_name: Union[str, Callable]):
    ...
    traced through::
        ...
        pippy.fx.wrap('my_custom_function')
