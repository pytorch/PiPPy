...
from pippy.fx.node import Argument, Target
from torch._jit_internal import boolean_dispatched
from pippy.fx.operator_schemas import _torchscript_type_to_python_type

from pippy.fx import Transformer
...
class AnnotateTypesWithSchema(Transformer):
    ...
    def call_function(
        ...
        if (
            self.annotate_functionals
            and target.__module__ == "torch.nn.functional"
        ):
            ...
            python_ret_type = self._extract_python_return_type(
                target_for_analysis
            )
        ...
        return_proxy.node.type = (
            return_proxy.node.type
            if return_proxy.node.type
            else python_ret_type
        )
        return return_proxy
        ...
        if self.annotate_get_attrs:
            ...
            if maybe_inferred_ts_type.success():
                ...
                attr_proxy.node.type = (
                    python_type
                    if not attr_proxy.node.type
                    else attr_proxy.node.type
                )
