...
def split_result_tensors(result: torch.Tensor, inputs: List[torch.Tensor]) -> List[torch.Tensor]:
    ...
    """
    # When fx tracer is running, x.shape[0] will be pippy.fx.Attribute but we
    # need an int even when tracing
    if isinstance(result, pippy.fx.Proxy):
        splits = [0] * len(inputs)
    else:
...
def may_depend_on(a: Node, b: Node, search_depth: int = 6):
    """
    Determine if one node depends on another in a pippy.fx.Graph.
