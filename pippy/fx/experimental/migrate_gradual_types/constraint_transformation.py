...
from pippy.fx.experimental.migrate_gradual_types.util import (
    gen_tensor_dims,
    gen_nat_constraints,
    gen_dvar,
)
...
def transform_get_item_tensor(constraint, counter):
    ...
    if len(resulting_tensor_dims) > 4:
        ...
        constraints = [
            BinConstraintT(constraint.input_var, TensorType(dims), op_eq),
            BinConstraintT(
                constraint.res, TensorType(resulting_tensor_dims), op_eq
            ),
            *nat_constraints,
            is_valid_index,
        ]
...
def generate_binconstraint_t(constraint, counter):
    ...
    if constraint.op == op_precision:
        if constraint.lhs == Dyn:
            ...
            if is_fully_static:
                return (
                    BinConstraintT(constraint.lhs, constraint.rhs, op_eq),
                    counter,
                )
            else:
                new_dims = []
                ...
                new_dim_constraints = (
                    [
                        ...
                        for new_dim, old_dim in zip(
                            new_dims, constraint.lhs.__args__
                        )
                    ]
                    + [
                        BinConstraintT(
                            constraint.rhs, TensorType(new_dims), op_eq
                        )
                    ]
                    + [
                        BinConstraintD(1, new_dim, op_leq)
    ...
    # matching
        ...
        return (
            Disj(
                [
                    Conj(conj),
                    BinConstraintT(
                        constraint.lhs, TensorType([d1, d2, d3, d4]), op_eq
                    ),
                ]
            ),
            counter,
        ...
        return (
            Disj([c_dyn, c_tensor_1, c_tensor_2, c_tensor_3, c_tensor_4]),
            counter,
        )
...
def generate_reshape(constraint, counter):
    ...
    c2_tensor4 = BinConstraintT(
        constraint.src, TensorType([d1, d2, d3, d4]), op_eq
    )
    ...
    if is_fully_static:
        ...
        c3_tensor1 = Disj(
            [
                d1_eq_dyn,
                (Conj([d1_neq_dyn, BinConstraintD(d1, Prod(target), op_eq)])),
            ]
        )
    ...
    # then there must be exactly one occurrence of dyn
        ...
        c3_tensor1 = Disj(
            [
                d1_eq_dyn,
                (Conj([d1_neq_dyn, is_dim_div_by_target(new_target, d1)])),
            ]
        )
        all_tensor_1 = Conj([c2_tensor1, c3_tensor1])
...
def calc_last_two_dims(constraint, d: List[DVar]):
    ...
    b3_dyn = Conj(
        [BinConstraintD(d[2], Dyn, op_eq), BinConstraintD(b3, Dyn, op_eq)]
    )
    b4_dyn = Conj(
        [BinConstraintD(d[3], Dyn, op_eq), BinConstraintD(b4, Dyn, op_eq)]
    )
    ...
    f3 = BinConstraintD(
        BinConstraintD(BinConstraintD(f1, f2, op_sub), 1, op_sub),
        stride[0],
        op_div,
    )
    ...
    f33 = BinConstraintD(
        BinConstraintD(BinConstraintD(f11, f22, op_sub), 1, op_sub),
        stride[1],
        op_div,
    )
...
def broadcast_dim(
    tensor_input1, tensor_input2, res1, res2, index, padding=False
):
    """
...
def gen_consistency_constraints(constraint: Constraint, counter: int):
    ...
    for i in range(1, MAX_TENSOR_RANK + 1):
        ...
        c_tensor_i = Conj(
            [
                BinConstraintT(
                    constraint.lhs, TensorType(new_dims_rhs_1), op_eq
                ),
                BinConstraintT(
                    constraint.rhs, TensorType(new_dims_rhs_2), op_eq
                ),
            ]
            + [
...
def gen_greatest_upper_bound(constraint: TGreatestUpperBound, counter: int):
    ...
    for i in range(1, MAX_TENSOR_RANK + 1):
        ...
        assert (
            len(c3tensor.__args__)
            == len(c1tensor.__args__)
            == len(c2tensor.__args__)
        )
