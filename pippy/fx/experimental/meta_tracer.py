...
def gen_constructor_wrapper(target):
    ...
    def wrapper(*args, **kwargs):
        ...
        def check_has_proxy(v):
            if isinstance(v, pippy.fx.Proxy):
                nonlocal proxy
                proxy = v

        pippy.fx.node.map_aggregate(args, check_has_proxy)
        pippy.fx.node.map_aggregate(kwargs, check_has_proxy)
...
class MetaProxy(pippy.fx.Proxy):
...
def proxys_to_metas(v):
    ...
    if isinstance(v, pippy.fx.Proxy):
        ...
        assert hasattr(
            v, "_tensor_meta"
        ), "MetaProxy does not have an associated meta"
        return v._tensor_meta
    return v


class MetaTracer(pippy.fx.Tracer):
    ...
    def create_proxy(
        ...
        try:
            args_metas = pippy.fx.node.map_aggregate(args, proxys_to_metas)
            kwargs_metas = pippy.fx.node.map_aggregate(kwargs, proxys_to_metas)
            ...
            assert isinstance(
                rv, pippy.fx.Proxy
            ), "Dont support composite output yet"
    ...
    def trace(
        self, root, meta_args: Dict[str, torch.Tensor], concrete_args=None
    ):
...
def symbolic_trace(
    ...
    gm = pippy.fx.GraphModule(tracer.root, graph, name)
    return gm
