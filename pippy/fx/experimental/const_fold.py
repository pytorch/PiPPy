...
class FoldedGraphModule(pippy.fx.GraphModule):
    """
    ...
    def __init__(
        ...
        const_subgraph: Optional[pippy.fx.Graph] = None,
        ...
        self.const_subgraph_module = (
            ...
            else pippy.fx.GraphModule(root, const_subgraph)
        )
...
def _inline_module(gm: pippy.fx.GraphModule, inline_mod_name: str):
    """
    ...
    assert isinstance(inline_mod, pippy.fx.GraphModule)
    ...
    replacement_mapping: Dict[pippy.fx.Node, pippy.fx.Node] = {}
    ph_count = 0
...
def split_const_subgraphs(
    module: Union[torch.nn.Module, pippy.fx.GraphModule],
    skip_folding_node_fn: Optional[Callable[[pippy.fx.Node], bool]] = None,
    ...
    graph.
    """
    if not isinstance(module, pippy.fx.GraphModule):
        mod_traced = pippy.fx.symbolic_trace(module)
    else:
        mod_traced = module
    ...
    const_nodes: Set[pippy.fx.Node] = set()
    ...
    def mod_partition(node: pippy.fx.Node):
    ...
    #    return add
    root_const_gm = pippy.fx.GraphModule(split, const_gm.graph)
