...
from pippy.events import Event, EventDependency, EventsContext, MemDumpEvent, Allocator
...
def generate_json(x: Any):
    if isinstance(x, str):
        ...
        return '{' + ','.join([f'{generate_json(str(key))}: {generate_json(val)}' for key, val in x.items()]) + '}'
    elif x is None:
        return '""'
    else:
        return f'{x}'
