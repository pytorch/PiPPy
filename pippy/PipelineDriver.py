...
def _get_value_on_remote(caller_rank, callee_rank, runlist_key, microbatch, rref):
    logging.info(
        f"[{callee_rank}][{microbatch}] Executing async transfer of value "
...
def async_transfer(rank, microbatch, scheduler_rref, rref_arg, arg_idx, runlist_key, max_outstanding):
    ...
    def bottom_half(fut):
        logging.info(
            f"[{rank}][{microbatch}] Completing transfer of value {rref_arg} from {rref_arg.owner().id} "
...
class PipeStageExecutor:
    ...
    * TODO: gradient checkpointing
    ...
    def _debug_print(self, to_file=False):
        ...
        s = (
            f"Executor instance {id(self)} for rank {self.local_rank}.\n"
    ...
    def worker_loop(self):
        while True:
            ...
            with self.ready_runlist_cv:
                ...
                for key in iter(self.ready_runlist.keys()):
                    ...
                    if (
                        ...
                        and self.ready_runlist[key].state == SchedState.WAITING
            ...
            logging.info(
                f"[{self.local_rank}][{work_item.microbatch_id}] Populating result of type {type(out_val)} "
                f"for {key}"
            )
    ...
    def invoke(self, unique_key: str, phase: Phase, args, kwargs, cur_microbatch: int, debug_str: str):
        ...
        logging.info(
            f"[{self.local_rank}][{cur_microbatch}] Invoke instantiated WorkItem {work_item} with key {unique_key}"
        )
        if len(rref_args) == 0:
            ...
            if work_item.state == SchedState.WAITING:
                logging.info(
                    f"[{self.local_rank}][{cur_microbatch}] Schedule limits max outstanding micro-bactches. Initializing as WAITING workitem"
                )
            else:
                logging.info(
                    f"[{self.local_rank}][{cur_microbatch}] No RRef arguments. Scheduling directly as READY workitem"
                )
            with self.ready_runlist_cv:
                logging.info(
                    f"[{self.local_rank}][{cur_microbatch}] Current ready runlist keys: {self.ready_runlist.keys()}"
                )
            ...
            with self.waiting_runlist_lock:
                logging.info(
                    f"[{self.local_rank}][{cur_microbatch}] Current waiting runlist keys: {self.waiting_runlist.keys()}"
                )
                assert (
        ...
        for arg_idx, rref_arg in enumerate(rref_args):
            logging.info(
                f"[{self.local_rank}][{cur_microbatch}] Launching asynchronous data transfer for RRef {arg_idx} {rref_arg}"
            )
...
class PipelineDriverBase:
    ...
    def _init_remote_executors(self):
        ...
        if len(executor_descriptors) < self.world_size:
            warnings.warn(
                f"Running pipeline with {len(executor_descriptors)} stages on world_size of {self.world_size}. "
        ...
        for rank, descr in zip(self.all_ranks, executor_descriptors):
            self.remote_stage_executor_rrefs[descr.name] = (
                rank,
                rpc.remote(rank, self.executor_class, (descr.mod, rank, descr.loss_mod, self.max_outstanding)),
            )
            if descr.loss_mod:
    ...
    def run(self, args, kwargs, chunks: int, _debug_mask_minibatches: bool = False):
        raise NotImplementedError(
            "PipelineDriverBase is an abstract base class, please use a concrete " "implementation class."
        )
...
class RemoteInterpreter(torch.fx.Interpreter):
    def __init__(
        self, remote_stage_executor_rrefs, module, cur_microbatch: int, args, kwargs, garbage_collect_values=True
    ):
    ...
    def call_module(self, target, args, kwargs):
        ...
        if target in self.remote_stage_executor_rrefs:
            ...
            logging.info(
                f"[root][{self.cur_microbatch}] Issuing {phase} " f"invocation for target {target} on rank {rank}"
            )
    ...
    def call_function(self, target, args, kwargs):
        if target is operator.getitem and isinstance(args[0], torch._C._distributed_rpc.PyRRef):
            ...
            logging.info(f"[root][{self.cur_microbatch}] Issuing sync invocation " f"on rank {rank}")
            invocation_key = f"{self.cur_microbatch}_{node.name}"
            return stage_executor.remote().invoke(
...
class PipelineDriverFillDrain(PipelineDriverBase):
    ...
    def run(self, args, kwargs, chunks: int, _debug_mask_minibatches: bool = False):
        ...
        args_split, kwargs_split = split_args_kwargs_into_chunks(
            args, kwargs, self.args_chunk_spec, self.kwargs_chunk_spec, chunks, _debug_mask_minibatches
        )
        ...
        for chunk in range(chunks):
            ...
            interp = RemoteInterpreter(
                self.remote_stage_executor_rrefs, self.pipe.split_gm, chunk, args_split[chunk], kwargs_split[chunk]
            )
