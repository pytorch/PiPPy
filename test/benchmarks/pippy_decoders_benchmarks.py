...
def run_all(pp_ranks, args):
    ...
    if args.rank == 0:
        ...
        print(f"model total number of params = {get_number_of_params(args.model) // 10 ** 6}M")
    ...
    inject_pipeline_forward(args.model, pipe_driver)
    ...
    # print(response)
    model_init_time = ((perf_counter()-start_time)*1000)+args.init_time
    hf_load_time = args.init_time
    pippy_compile_time = ((perf_counter()-start_time)*1000)
    input_ids = generate_input_ids_batch(args.batch_size, args.seq_len)
    input_ids = input_ids.to(args.device)
    #Run benchmarks
    total_time = benchmark(args.model,input_ids,args)
    time_per_token = total_time/args.max_tokens
    print("benchmark is done and avegrage time per tokens is {}".format(time_per_token))
    ...
    if os.path.exists(args.log_filename):
        ...
        output_file.write(
            "model_name, batch_size,chunks, seq_len, max_tokens,time_per_token, total_time, total model init time, hf_load_time, pippy_compile\n"
        )
...
if __name__ == "__main__":
    ...
    supported_model_categories = ["opt", "gpt2", "bloom", "EleutherAI/gpt", "codegen", "GPT", "bigscience"]
    # For example:
    # "facebook/opt-350m"
    # "gpt2"
    # "bigscience/bloom-3b"
    ...
    init_time_start = perf_counter()
    if any([m in args.model_name for m in supported_model_categories]):
        print(f"Loading model {args.model_name}")
        if args.index_filename is not None:
            with init_empty_weights():
                args.model = AutoModelForCausalLM.from_pretrained(args.model_name, use_cache=False, torch_dtype=dtype)
        else:
            args.model = AutoModelForCausalLM.from_pretrained(args.model_name, use_cache=False, torch_dtype=dtype)
    else:
        raise ValueError(f"Unsupported model: {args.model_name}")

    init_time = (perf_counter()-init_time_start)*1000
