...
from fx.test_subgraph_rewriter import (
    TestSubgraphRewriter,
)  # noqa: F401  # pylint: disable=unused-import
from fx.test_dce_pass import (
    TestDCE,
)  # noqa: F401  # pylint: disable=unused-import
from fx.test_fx_const_fold import (
    TestConstFold,
)  # noqa: F401  # pylint: disable=unused-import
from fx.test_fx_param_shape_control_flow import (
    TestConstParamShapeInControlFlow,
)  # noqa: F401  # pylint: disable=unused-import
from fx.test_pass_infra import (
    TestPassManager,
)  # noqa: F401  # pylint: disable=unused-import

if sys.version_info >= (3, 7):
    from fx.test_gradual_type import (
        AnnotationsTest,
    )  # noqa: F401  # pylint: disable=unused-import
if sys.version_info >= (3, 7):
    from fx.test_gradual_type import (
...
class TestFX(JitTestCase):
    ...
    def test_trace_multiple_funcs(self):
        ...
        torch.testing.assert_close(
            GraphModule(f, tracer.trace(f))(x, y), f(x, y)
        )
        ...
        def lower_to_elementwise_interpreter(
            ...
            for n in mod.graph.nodes:
                ...
                if n.op == "placeholder":
                    ...
                    assert target in target_to_name, (
                        "Unsupported call target " + target
                    )
                    arg_names = []
                    for arg in args:
    ...
    def test_copy_no_remap(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError, "does not belong to this Graph"
        ):
            copied.lint()
    ...
    def test_shape_prop_layout_3d(self):
        ...
        x_channels_last_3d = x_3d.contiguous(
        ...
        for node in traced_3d.graph.nodes:
            ...
            if node.op in {"placeholder"}:
                self.assertEqual(
                    node.meta["tensor_meta"].memory_format,
    ...
    def test_torchbind_class_attribute_in_fx(self):
        ...
        class FooBar1234(torch.nn.Module):
            def __init__(self):
                ...
                self.f = torch.classes._TorchScriptTesting._StackString(
                    ["3", "4"]
                )
    ...
    def test_named_tuple_inlined(self):
        ...
        for node in traced.graph.nodes:
            if node.op == "placeholder":
                ...
                node.op == "call_function" and node.target == wrapped_named_tup
            ):
        ...
        self.assertEqual(
            _format_arg(call_func.kwargs["p2"]), "Pair(x=3.4, y=%inp)"
        )
    ...
    def test_custom_traceback_raised_when_exception_source_is_graphmodule(self):
        ...
        with traced.graph.inserting_before(out):
            relu_out = traced.graph.call_method(
                method_name="relu", args=(out.args[0],)
            )
        out.args = (relu_out,)
    ...
    def test_custom_traceback_not_raised_when_exception_source_is_submodule(
        self,
    ):
        class M(torch.nn.Module):
            def __init__(self):
    ...
    def test_submodule_manipulation_API(self):
        ...
        def parameter_exists(gm: GraphModule, path: str) -> bool:
            return any(
                path == name for name, _ in gm.named_parameters()
        ...
        with self.assertRaisesRegex(
            AttributeError, "has no attribute " "`conv`"
        ):
        ...
        with self.assertRaisesRegex(
            AttributeError, "is not an " "nn.Parameter"
        ):
            a.get_parameter("net_b.buf")
        with self.assertRaisesRegex(
            AttributeError, "has no attribute " "`param`"
        ):
        ...
        with self.assertRaisesRegex(
            AttributeError, "has no attribute " "`buf`"
        ):
    ...
    def test_tracing_graphmodules_as_leaf_submodules(self):
        ...
        match = [
            ...
            if n.op == "call_module" and n.target == "a"
        ]
    ...
    def _test_graph_module_init_buffer_param_copied(self, use_dict_init: bool):
        ...
        mod_traced_new = GraphModule(
            {"my_buff": orig_buff, "my_param": orig_param}
            if use_dict_init
            else mod,
            mod_traced.graph,
        )
    ...
    def test_annotations_with_non_torch_reference_and_internal_forward_references(
        self,
    ):
        class A:
            def __call__(self, x: torch.Tensor):
    ...
    def test_annotations_empty_tuple(self):
        ...
        FileCheck().check("Tuple[()]").check("Tuple[str, Tuple[()]]").run(
            scripted.code
        )
    ...
    def test_pytree(self):
        ...
        def verify_pytree(f, inp):
            val = pytree.tree_map(
                lambda x: torch.randn(3) if x == PH else x, inp
            )
            ...
            bare_fx.recompile()
            self.assertEqual(
                nf.graph.process_outputs(
                    bare_fx(*nf.graph.process_inputs(val))
                ),
                orig_out,
            )
            ...
            assert (
                sum([i.op == "placeholder" for i in nf.graph.nodes])
                == num_flat_args
            )
...
def forward(self, args_list: List[torch.Tensor]){maybe_return_annotation}:
    {', '.join(var.name for var in free_vars)} = args_list"""
        ...
        self.assertEqual(nf(vals), f(*vals))
        self.assertEqual(
            nf.graph.process_outputs(bare_fx(*nf.graph.process_inputs(vals))),
            f(*vals),
        )
...
def forward(self, args_list: List[torch.Tensor]){maybe_return_annotation}:
    ...
    def test_map_aggregate_doesnt_traverse_size(self):
        ...
        data = (
            ...
            size,
            {
                "tensor": torch.empty(4, 5),
                "size": size,
                "list": [size, (size,), torch.empty(5, 6)],
            },
        )
...
class TestOperatorSignatures(JitTestCase):
    ...
    def test_get_torch_func_signature_exhaustive(self, device, dtype, op):
        if not isinstance(op.op, types.BuiltinFunctionType):
            raise unittest.SkipTest(
                "This path doesn't work on Python functions"
            )
        ...
        for sample_input in sample_inputs_itr:
            ...
            for schema in schemas:
                ...
                raise RuntimeError(
                    f"Did not match any schemas for op {op.name}!"
                )
...
class TestFXAPIBackwardCompatibility(JitTestCase):
        ...
        for k, v in signature.parameters.items():
            ...
            def default_val_str(val):
                if isinstance(val, (tuple, list)):
                    ...
                    str_pieces.append(
                        ", ".join(default_val_str(v) for v in val)
                    )
            ...
            if v.kind == inspect.Parameter.VAR_POSITIONAL:
                ...
                maybe_stars = "**"
            arg_strs.append(
                f"{maybe_stars}{k}{maybe_type_annotation}{maybe_default}"
            )
    ...
    def _annotation_type_to_stable_str(self, t, sig_str):
        ...
        if origin in {typing.Union}:
            ...
            if len(contained) == 2 and (contained[0] is type(None)) ^ (
                ...
                not_none_param = (
                    contained[0]
                    if contained[0] is not type(None)
                    else contained[1]
                )
    ...
    def test_public_api_surface(self):
        ...
        check_symbols_have_bc_designation(
            pippy.fx.passes, ["pippy", "fx", "passes"]
        )
...
class TestFunctionalTracing(JitTestCase):
    ...
    LEN_ERROR = (
        RuntimeError,
        r"'len' is not supported in symbolic tracing by default",
    )
