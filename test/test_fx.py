...
class TestFX(JitTestCase):
    def setUp(self):
        ...
        self.orig_tracer_mutable_flag = pippy.fx.proxy.TracerBase.check_mutable_operations
    ...
    def tearDown(self):
        super().tearDown()
        pippy.fx.proxy.TracerBase.check_mutable_operations = self.orig_tracer_mutable_flag
    ...
    def test_graph_unique_names_manual(self):
        ...
        b : pippy.fx.Node = graph.create_node('call_module', 'linear_mod', args=(a,), name='foo_1_1')
        c : pippy.fx.Node = graph.create_node('get_attr', 'y_attr', name='foo_1')
        d : pippy.fx.Node = graph.create_node('call_function', operator.add, args=(b, c))
        graph.output(d)
    ...
    def test_construct_root_dict(self):
        graph : pippy.fx.Graph = pippy.fx.Graph()
        a : pippy.fx.Node = graph.create_node('placeholder', 'x')
        b : pippy.fx.Node = graph.create_node('call_module', 'foo.bar.baz', args=(a,))
    ...
    def test_wrong_target_type(self):
        ...
        with self.assertRaises(ValueError):
            n = pippy.fx.Node(graph=graph, name='foo', op='call_function', target='foo',
                              args=(), kwargs={})
    ...
    def test_typename_print(self):
        ...
        b : pippy.fx.Node = graph.create_node('call_function', target=torch.relu, args=(x,),
    ...
    def test_inf_nan_kwds(self):
        graph : pippy.fx.Graph = pippy.fx.Graph()
        x : pippy.fx.Node = graph.create_node('placeholder', 'x')
        b : pippy.fx.Node = graph.create_node('call_function', operator.add, (x, float('inf')), {}, name='inf')
        c : pippy.fx.Node = graph.create_node('call_function', operator.add, (x, float('nan')), {}, name='nan')
        graph.output((b, c))
    ...
    def test_insertion_point(self):
        ...
        with graph.inserting_before(b):
            neg : pippy.fx.Node = graph.call_function(the_function=torch.neg, args=(x,))
            _, *relu_args = b.args
    ...
    def test_update_kwargs_api(self):
        ...
        x : pippy.fx.Node = graph.create_node('placeholder', 'x')
        y : pippy.fx.Node = graph.create_node('placeholder', 'y')
        b : pippy.fx.Node = graph.create_node('call_function', target=torch.relu, kwargs={'input': x})
        output : pippy.fx.Node = graph.output(b)
    ...
    def test_inline_graph(self):
        ...
        combined_module = pippy.fx.GraphModule(torch.nn.Module(), combined_graph)
...
class TestFXAPIBackwardCompatibility(JitTestCase):
    ...
    def test_public_api_surface(self):
        ...
        non_back_compat_strs = [
            s for s in non_back_compat_strs if s.startswith('pippy.fx') and not s.startswith('pippy.fx.experimental')]
