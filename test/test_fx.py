...
class TestFX(JitTestCase):
    ...
    @unittest.skipIf(
        sys.version_info >= (3, 10), "Does not work on Python-3.10"
    )
...
class TestVisionTracing(JitTestCase):
    ...
    def generate_segmentation_tests(cls):
        for k in torchvision_models.list_models(
            module=torchvision_models.segmentation
        ):
    ...
    def generate_detection_tests(cls):
        for k in torchvision_models.list_models(
            module=torchvision_models.detection
        ):
    ...
    def generate_video_tests(cls):
        for k in torchvision_models.list_models(
            module=torchvision_models.video
        ):
