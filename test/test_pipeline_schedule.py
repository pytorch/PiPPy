...
class TestPipelineSchedule(MultiProcessTestCase):
    ...
    # PIPPY_VERBOSITY=DEBUG pytest test_pipeline_schedule.py -vsk test_interleaved_1f1b_bug
    @skip_if_lt_x_gpu(4)
    def test_interleaved_1f1b_bug(self):
        ...
        batch = torch.rand((8, 4), device=device)
        num_microbatches = 8
        stages = self._create_virtual_pipeline_stages(
            model, torch.rand((1, 4)).to("meta"), device, 2, num_microbatches=num_microbatches
        )
        schedule = ScheduleInterleaved1F1B(
            stages,
            num_microbatches,
        ...
        if self.rank == 0:
            ...
            target = torch.rand((8,), device=device)
            losses = []
