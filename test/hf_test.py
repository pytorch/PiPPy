        ...
        for i in range(model.config.num_hidden_groups):
            for j in range(model.config.inner_group_num):
                annotate_split_points(
                    model, {f"encoder.albert_layer_groups.{i}.albert_layers.{j}": PipeSplitWrapper.SplitPoint.BEGINNING}
                )
    ...
    def scope(model_cls, replicate):
        def test_case(self):
            ...
            multi_use_param_config = (
                MultiUseParameterConfig.REPLICATE if replicate else MultiUseParameterConfig.TRANSMIT
            )
            model_pipe = Pipe.from_tracing(model, multi_use_param_config, tracer=hf_tracer, concrete_args=concrete_args)
