...
from pippy.fx.experimental.meta_tracer import (
...
from pippy.fx.experimental.migrate_gradual_types.constraint_generator import (
    ConstraintGenerator,
)
from pippy.fx.experimental.migrate_gradual_types.constraint_transformation import (
...
from pippy.fx.experimental.migrate_gradual_types.transform_to_z3 import (
...
class HFOperations(unittest.TestCase):
    ...
    def test_conditional_ne_1(self):
        ...
        class BasicBlock(torch.nn.Module):
            ...
            def forward(
                self, x: TensorType([32, 4, 4]), y: TensorType([32, 4, 4])
            ):
                size_5 = x.size()
                getitem_7 = size_5[0]
                getitem_8 = size_5[1]
    ...
    def test_bmm(self):
        class BasicBlock(torch.nn.Module):
            ...
            def forward(
                self, x: TensorType([Dyn, 2, 3]), y: TensorType([1, 3, 2])
            ):
                bmm = torch.bmm(x, y)
                return bmm
    ...
    def test_bmm3(self):
        class BasicBlock(torch.nn.Module):
            ...
            def forward(
                self, x: TensorType([2, 3, 3]), y: TensorType([1, 3, 2])
            ):
                bmm = torch.bmm(x, y)
                return bmm
    ...
    def test_ne_int_long_type_as(self):
        class BasicBlock(torch.nn.Module):
            ...
            def forward(
                self, x: TensorType([Dyn, Dyn]), y: TensorType([Dyn, Dyn])
            ):
        ...
        self.assertEquals(s.check(), z3.sat)
        actual_shape = (
            BasicBlock().forward(torch.rand(2, 4), torch.rand(2, 4)).shape
        )
    ...
    def test_cumsum(self):
        ...
        symbolic_traced: pippy.fx.GraphModule = meta_symbolic_trace(
    ...
    def test_arange(self):
        ...
        self.assertEqual(
            s.model()[arange_result].arg(0).arg(1).as_long(), B.size()[0]
        )
    ...
    def test_embedding_2(self):
        class BasicBlock(torch.nn.Module):
            ...
            def forward(
                self, x: TensorType([2, 4]), y: TensorType([Dyn, 1024])
            ):
        ...
        B = (
            BasicBlock()
            .forward(
                torch.ones([2, 4], dtype=torch.long), torch.rand(256008, 1024)
            )
            .size()
        )
        ast_rewriter = RewritingTracer()
...
class ComposeOperationsGradualTypes(unittest.TestCase):
    ...
    def test_add_reshape_1(self):
        class BasicBlock(torch.nn.Module):
            ...
            def forward(self, x: Dyn, y: Dyn):
                return torch.add(
                    torch.reshape(x, (1, 2)), torch.reshape(y, (2, 2))
                )
    ...
    def test_add_reshape_2(self):
        class BasicBlock(torch.nn.Module):
            ...
            def forward(self, x: Dyn, y: Dyn):
                return torch.add(
                    torch.reshape(x, (-1, 2)), torch.reshape(y, (2, 2, 2))
                )
    ...
    def test_conv_reshape_add_0(self):
        class BasicBlock(torch.nn.Module):
            ...
            def forward(self, x: Dyn, y: Dyn):
                return torch.add(
                    self.conv1(torch.reshape(x, (1, 2, 10, 20))), y
                )
...
class TestSingleOperation(unittest.TestCase):
    ...
    def test_add_size_3(self):
        class BasicBlock(torch.nn.Module):
            ...
            def forward(
                self,
                x: TensorType([Dyn, Dyn, Dyn]),
                y: TensorType([Dyn, Dyn, Dyn]),
            ):
                return torch.add(x, y)
    ...
    def test_add_padding_6(self):
        class BasicBlock(torch.nn.Module):
            ...
            def forward(
                self, x: TensorType([Dyn]), y: TensorType([Dyn, Dyn, Dyn])
            ):
                return torch.add(x, y)
    ...
    def test_conv2D_maxpool2d_flatten(self):
        ...
        input = z3.Const(1, tensor_type)
        solver.add(
            input == tensor_type.tensor4(D(1, 4), D(1, 3), D(1, 32), D(1, 32))
        )
        solver.check()
    ...
    def test_conv2D_maxpool2d_flatten_unsat(self):
        ...
        input = z3.Const(1, tensor_type)
        solver.add(
            input == tensor_type.tensor4(D(1, 4), D(1, 3), D(1, 32), D(1, 45))
        )
        self.assertEquals(solver.check(), z3.unsat)
    ...
    def test_type_check_flatten(self):
        ...
        solver.add(
            x == tensor_type.tensor4(D(1, 2), D(1, 3), D(0, s1), D(1, 5))
        )
        self.assertEquals(solver.check(), z3.sat)
...
class TestResNet(unittest.TestCase):
    ...
    def test_resnet50(self):
        ...
        input = z3.Const(1, tensor_type)
        solver.add(
            input == tensor_type.tensor4(D(1, 1), D(1, 3), D(1, 224), D(1, 224))
        )
        self.assertEquals(solver.check(), z3.sat)
        assert solver.model()[linear] == tensor_type.tensor2(
            D(1, res[0]), D(1, res[1])
        )
    ...
    def test_resnet502(self):
        ...
        solver.add(
            input
            == tensor_type.tensor4(D(1, batch), D(1, 3), D(1, 224), D(1, 224))
        )
        solver.add(batch > 4)
        solver.check()
...
class TestAlexNet(unittest.TestCase):
    def test_alexnet1(self):
        ...
        solver.add(
            input
            == tensor_type.tensor4(D(1, 10), D(1, 3), D(1, 227), D(1, 227))
        )
        self.assertEquals(solver.check(), z3.sat)
        ...
        assert solver.model()[flatten] == tensor_type.tensor2(
            D(1, 10), D(1, 9216)
        )
