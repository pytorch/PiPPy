...
from pippy.fx.tensor_type import Dyn, is_consistent, is_more_precise, TensorType
...
class AnnotationsTest(TestCase):
    ...
    def test_consistency(self):
        ...
        """
        self.assertTrue(
            is_consistent(TensorType((1, 2, 3)), TensorType((1, Dyn, 3)))
        )
        self.assertTrue(is_consistent(int, Dyn))
        self.assertTrue(is_consistent(int, int))
        self.assertFalse(
            is_consistent(TensorType((1, 2, 3)), TensorType((1, 2, 3, 5)))
        )
    ...
    def test_precision(self):
        ...
        """
        self.assertTrue(
            is_more_precise(TensorType((1, 2, 3)), TensorType((1, Dyn, 3)))
        )
...
class TypeCheckerTest(TestCase):
    def test_type_check_add_with_broadcast(self):
        class M(torch.nn.Module):
            def forward(
                self, x: TensorType((1, 2, 3, Dyn)), y: TensorType((2, 3, 4))
            ):
                return torch.add(x, y)
    ...
    def test_type_check_add_false(self):
        class M(torch.nn.Module):
            def forward(
                self, x: TensorType((1, 2, 3, Dyn)), y: TensorType((1, 2, 3))
            ):
                return torch.add(x, y)
    ...
    def test_type_check_add_true(self):
        class M(torch.nn.Module):
            def forward(
                self, x: TensorType((1, 2, Dyn)), y: TensorType((1, 2, 3))
            ):
                return torch.add(x, y)
    ...
    def test_resnet50(self):
        ...
        for n1, n2 in zip(gm_static.graph.nodes, gm_run.graph.nodes):
            assert is_consistent(
                n1.type, TensorType(n2.meta["tensor_meta"].shape)
            )
