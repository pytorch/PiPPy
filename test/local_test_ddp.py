...
def set_grad_in_executor(executor, qualname, value):
    mod = executor.local_value().mod
    if isinstance(mod, torch.nn.parallel.DistributedDataParallel):
        param = mod.module.get_parameter(qualname)
    else:
...
def run_worker(rank, world_size, args):
    ...
    if rank >= 0 and rank // args.dp_group_size == 0:
        args.rank = rank
        run_main(args, pp_ranks_per_dp_group[rank])
    rpc.shutdown()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dp_group_size', type=int, default=int(os.getenv("WORLD_SIZE", 4)))
    parser.add_argument('--pp_group_size', type=int, default=int(os.getenv("WORLD_SIZE", 3)))
