...
def get_layer_device_map(model):
    ...
    for name, module in model.named_modules():
        if hasattr(module, 'weight'):
            layer_device_map[name] = str(module.weight.device)
...
def run_all(args):
    ...
    if rank==0:
        print_submodules(args.model)
    input_ids = input_ids.to(args.device)
    
    print("==============================================")
    TP_time_start = time.perf_counter()
    if "llama" in args.model_name:
        tp_llama(args.model,dm)
    elif "opt" in args.model_name:
        tp_opt(args.model,dm)
    TP_time_end = time.perf_counter()-TP_time_start
    max_mem = "50GB" # Max memory per GPU
    
    
    # checkpoint_file = "/data/home/hamidnazeri/PiPPy/examples/inference/model/models--meta-llama--Llama-2-7b-chat-hf/snapshots/08751db2aca9bf2f7f80d2e516117a53d7450235/"
    checkpoint_file = "/data/home/hamidnazeri/PiPPy/examples/inference/model/models--facebook--opt-350m/snapshots/cb32f77e905cccbca1d970436fb0f5e6b58ee3c5"
    # if args.model:
        # args.model.tie_weights()
    custom_device_map = infer_auto_device_map(args.model.model,max_memory={0: max_mem, 1: max_mem, 2:max_mem, 3:max_mem, 4:max_mem, 5:max_mem, 6:max_mem,7:max_mem})
    print("custom device map", custom_device_map)
    print("==============================================")
    
    model = load_checkpoint_and_dispatch(args.model, checkpoint=checkpoint_file, device_map=custom_device_map)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--meta_device', type=bool, default=False)
