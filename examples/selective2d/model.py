# Original code from https://github.com/karpathy/nanoGPT
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        q = (
            self.q(x)
            .split(self.n_embd // self.tp_size, dim=2)[0]
            .view(B, T, self.n_head // self.tp_size, C // self.n_head)
            .transpose(1, 2)
        )  # (B, nh, T, hs)
        k = (
            self.k(x)
...
class GPT(nn.Module):
    def __init__(self, mesh, config, device, pp_size=2):
        ...
        self.transformer = nn.ModuleDict(
            dict(
                ...
                h=nn.ModuleList(
                    [Block(mesh, config) for _ in range(config.n_layer)]
                ),
    ...
    def forward(self, idx, targets=None):
        ...
        pos = torch.arange(
            0, self.config.block_size, dtype=torch.long, device=self.device
        ).unsqueeze(0)
