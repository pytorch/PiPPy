...
def dist_addmm(
    ...
    assert is_replicate(input_placement), "only support replication now"

    # only implemented combo with no comm for now
    # TODO: implement all combinations
    if is_shard(mat1_placement, dim=0) and is_replicate(mat2_placement):
    ...
    elif is_replicate(mat1_placement) and is_shard(mat2_placement, dim=1):
        local_res = local_input.addmm(
            local_mat1, local_mat2, beta=beta, alpha=alpha
        )
        return Tensor.from_local(local_res, device_mesh, mat2.placements)
    elif is_replicate(mat1_placement) and is_replicate(mat2_placement):
        local_res = local_input.addmm(
            local_mat1, local_mat2, beta=beta, alpha=alpha
        )
...
def dist_mm(mat1: Tensor, mat2: Tensor) -> Tensor:
    ...
    if is_shard(mat1_placement, dim=0) and is_replicate(mat2_placement):
        ...
        local_res = local_mat1.mm(local_mat2)
        return Tensor.from_local(local_res, device_mesh, mat2.placements)
    elif is_shard(mat1_placement, dim=1) and is_shard(mat2_placement, dim=0):
        local_res = local_mat1.mm(local_mat2)
...
def dist_t(self: Tensor) -> Tensor:
    ...
    new_shard_dim = 1 if is_shard(mat_placement, dim=0) else 0
    return Tensor.from_local(
