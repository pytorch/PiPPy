...
def _convert_to_distributed_graph(
    ...
    input_schema: Schema = Schema(mesh=param_schema.mesh, placements=[Shard(0)])
    schemas: List[Schema] = [
        param_schema if is_param(inp) else input_schema for inp in inputs
    ]

    node_to_obj: Dict[fx.Node, object] = {}
    # map local op node in traced_f to its corresponding subgraph of DTensor ops.
    node_replacements: Dict[torch.fx.Node, torch.fx.GraphModule] = {}
    ...
    for i, node in enumerate(gm.graph.nodes):
        ...
        if node.op == OP.PLACEHOLDER:
            ...
            node_to_obj[node] = DTensor.from_local(
                inputs[i], schemas[i].mesh, schemas[i].placements
            )
...
def distribute(
    dist_graph: DistributedGraph, param_schema: Schema
) -> DistributedGraph:
    _convert_to_distributed_graph(
        gm=dist_graph.fwd_graph_modules[0],
        inputs=dist_graph.fwd_aot_module_inputs[0],
        orig_module=dist_graph.orig_module,
        param_schema=param_schema,
    )
    _convert_to_distributed_graph(
        gm=dist_graph.bwd_graph_modules[0],
        inputs=dist_graph.bwd_aot_module_inputs[0],
        orig_module=dist_graph.orig_module,
        param_schema=param_schema,
    )
    return dist_graph
