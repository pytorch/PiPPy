...
def graph_optimization_pass(run_after: Iterable[str] = tuple()) -> Callable:
    ...
    def pass_wrapper(func) -> Callable:
        for name in run_after:
            _run_before_sets[name].add(func.__name__)

        def pass_func(
            self: "DistGraphOptimization", *args, **kwargs
...
class DistGraphOptimization:
    ...
    def distribute(self, param_schema: Schema) -> "DistGraphOptimization":
        # ``shard`` may be a better naming but it is widely used. Use distribute
        # for now.
        distribute(self, param_schema)
        return self

    @graph_optimization_pass(run_after=["replicate"])
