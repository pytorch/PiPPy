...
class FsspecWriter(StorageWriter):
    ...
    * File creation is atomic
    ...
    def finish(
        ...
        metadata_path = os.path.join(self.path, ".metadata.tmp")
        with fsspec.open(metadata_path, "wb") as metadata_file:
            pickle.dump(metadata, metadata_file)
            if self.sync_files:
                os.sync()

        os.rename(metadata_path, os.path.join(self.path, ".metadata"))
...
class FsspecReader(StorageReader):
    ...
    def _slice_file(self, file, sinfo: _StorageInfo):
        return SlicedBufferedReader(
            io.FileIO(file, closefd=True), sinfo.offset, sinfo.length
        )
        ...
        for relative_path, reqs in per_file.items():
            ...
            with fsspec.open(abs_path, "rb") as file:
                ...
                for req in reqs:
                    ...
                    file_slice = self._slice_file(abs_path, item_md)
